...
function showFinanceSidebar() {
  var html = HtmlService.createHtmlOutput(`
    ...
    <body>
      ...
      <button class="btn" onclick="activateTab('Credit_Cards')">💳 Credit Card Usage</button>
      <button class="btn" onclick="activateTab('Bank_Accounts')">💳 Debit Card & Bank</button>
      <button class="btn" onclick="activateTab('Cash_Flow')">💰 Cash Flow</button>
      <button class="btn" onclick="activateTab('Investments')">📈 Trade & Investments</button>
      <button class="btn" onclick="activateTab('Budgets')">🎯 Budget vs Actual</button>
...
/**
 * Automatically creates all 10 tab modules in the Google Sheet matching the app sidebar
 */
function setupSheetStructure() {
  ...
  var tabs = [
    ...
    { name: 'Credit_Cards', headers: ['ID', 'Card Name', 'Bank', 'Network', 'Limit', 'Outstanding', 'Due Date', 'Statement Date', 'Last 4'] },
    { name: 'Bank_Accounts', headers: ['ID', 'Account Name', 'Bank', 'Account Type', 'Balance', 'Account No'] },
    { name: 'Cash_Flow', headers: ['ID', 'Date', 'Type (Income/Expense/Investment)', 'Category', 'Amount', 'Payment Method', 'Account', 'Description'] },
    { name: 'Investments', headers: ['ID', 'Symbol', 'Asset Name', 'Type', 'Quantity', 'Avg Buy Price', 'Current Price', 'Invested Amount', 'Current Value', 'P&L'] },
    { name: 'Budgets', headers: ['ID', 'Category', 'Monthly Budget', 'Current Spent'] },
